...
if __name__ == '__main__':
    ...
    try:
        ...
        while not STOP:
            ...
            # them to dual_hand_action_array, but its DDS Write() silently fails
            # because CycloneDDS internal threads are not inherited after fork().
            # We therefore read the shared array and publish from this (main) process.
            if args.ee == "dex3" and args.input_mode == "hand":
                with dual_hand_data_lock:
                    _hand_act = np.array(dual_hand_action_array[:])
                if np.any(_hand_act != 0.0):
                    hand_ctrl.publish_from_main_process(_hand_act[:7], _hand_act[7:])
