...
class Dex3_1_Controller:
    def __init__(self, left_hand_array_in, right_hand_array_in, dual_hand_data_lock = None, dual_hand_state_array_out = None,
        ...
        # Initialize main-process message templates for DDS publishing.
        # CycloneDDS is not fork-safe: publishers created before fork() cannot
        # transmit data from the child process because DDS internal send threads
        # are not inherited.  We therefore also publish from the main process.
        self._init_main_process_msgs()

        logger_mp.info("Initialize Dex3_1_Controller OK!")

    def _init_main_process_msgs(self):
        """Create HandCmd_ message templates in the main process so that
        ``publish_from_main_process`` can send hand commands via the original
        (working) DDS publishers."""
        q, dq, tau, kp, kd = 0.0, 0.0, 0.0, 1.5, 0.2

        self._main_left_msg = unitree_hg_msg_dds__HandCmd_()
        for id in Dex3_1_Left_JointIndex:
            ris = self._RIS_Mode(id=id, status=0x01)
            self._main_left_msg.motor_cmd[id].mode = ris._mode_to_uint8()
            self._main_left_msg.motor_cmd[id].q = q
            self._main_left_msg.motor_cmd[id].dq = dq
            self._main_left_msg.motor_cmd[id].tau = tau
            self._main_left_msg.motor_cmd[id].kp = kp
            self._main_left_msg.motor_cmd[id].kd = kd

        self._main_right_msg = unitree_hg_msg_dds__HandCmd_()
        for id in Dex3_1_Right_JointIndex:
            ris = self._RIS_Mode(id=id, status=0x01)
            self._main_right_msg.motor_cmd[id].mode = ris._mode_to_uint8()
            self._main_right_msg.motor_cmd[id].q = q
            self._main_right_msg.motor_cmd[id].dq = dq
            self._main_right_msg.motor_cmd[id].tau = tau
            self._main_right_msg.motor_cmd[id].kp = kp
            self._main_right_msg.motor_cmd[id].kd = kd

    def publish_from_main_process(self, left_q_target, right_q_target):
        """Publish hand commands from the main process (DDS-safe).

        Call this from the main event loop with retargeted joint values
        obtained from ``dual_hand_action_array``."""
        for idx, id in enumerate(Dex3_1_Left_JointIndex):
            self._main_left_msg.motor_cmd[id].q = left_q_target[idx]
        for idx, id in enumerate(Dex3_1_Right_JointIndex):
            self._main_right_msg.motor_cmd[id].q = right_q_target[idx]

        self.LeftHandCmb_publisher.Write(self._main_left_msg)
        self.RightHandCmb_publisher.Write(self._main_right_msg)
